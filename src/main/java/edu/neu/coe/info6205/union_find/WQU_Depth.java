 ...
 */
public class WQU_Depth implements UF {
    /**
     ...
     */
    public WQU_Depth(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        depth = new int[n];
    ...
    public void union(int p, int q) {
    	...
    	int parentOfP = getParent(p);
    	int parentOfQ = getParent(q);
        mergeComponents(parentOfP, parentOfQ);
//        if(updateHeight && (parentOfQ != q)) {
//        	updateDepth(q, 1);
//        }
        count--;
    }
...
//    		int noofSites = Integer.parseInt(args[0]);
    		WQU_Depth data = new WQU_Depth(10);
